...
if(!class_exists("Gold_Plugins_Media_Button")){
	class Gold_Plugins_Media_Button
	{
		...
		function __construct($button_label = '', $button_dashicon = '', $wrapper_class = '')
		{
			$this->add_hooks();
		 ...
		 */
		function enqueue_admin_scripts($force = false)
		{
			global $pagenow;
						
			if( $force || 'edit.php' == $pagenow || 'post-new.php' == $pagenow  || 'post.php' == $pagenow || (('admin.php' == $pagenow) && (strpos($_GET['page'], 'shortcode-generator') !== false)) )//RWG: add check for SC generator pages
			{
		...
		function add_media_button($context)
		{
			...
			$context .= sprintf('<div class="gp_media_button_group %s">', $this->wrapper_class);
			$btn = '';
		...
		function force_enqueue_admin_scripts()
		{
