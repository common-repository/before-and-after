var b_a_gold_plugins_init_upgrade_slideshow = function () {
	$ = jQuery;
...
var b_a_gold_plugins_init_coupon_box = function () {
		...
		$form.bind('submit', function () {
			...
			jQuery.ajax(
			{
				...
				success: function (dat) {
					setTimeout(function () {
						b_a_gold_plugins_ml_ajax_success($form, btn);
...
var b_a_gold_plugins_ml_ajax_success = function ($form, btn, speed) {
	if(typeof(speed) == 'undefined') {
		speed = 400;
	}
	$form.find('.fields_wrapper').slideUp(speed);
	$cpn_box = b_a_gold_plugins_get_coupon_box_new();
	btn.val('Coupon sent!');
	$cpn_box.fadeIn((speed * 2));
	btn.after($cpn_box);
};

var b_a_gold_plugins_get_coupon_box_new = function () {
	var coupon_html = 
